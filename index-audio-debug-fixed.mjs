...
// FIXED: Enhanced Player Configuration untuk Audio Stability
console.log("🎵 Discord Music Bot - AUDIO DEBUG FIXED VERSION");

const player = new Player(client, {
    // FIXED: Enhanced ytdl options untuk audio stability
    ytdlOptions: {
    ...
    // FIXED: Audio player options untuk mencegah audio cut-off
    audioPlayerOptions: {
        behaviors: {
            noSubscriber: 'pause',
    ...
    // FIXED: Connection options untuk voice stability
    connectionOptions: {
        selfDeaf: true,
        selfMute: false,
    },
...
// FIXED: Enhanced Event Handlers dengan Audio Debugging
player.events.on("playerStart", (queue, track) => {
    ...
    console.log(`🎵 Audio Resource: ${queue.node.resource ? 'Created' : 'Missing'}`);
...
// FIXED: Enhanced audio monitoring
...
// FIXED: Enhanced playerFinish event dengan audio debugging
...
// FIXED: Enhanced error handling dengan audio debugging
...
// FIXED: Connection state monitoring dengan debugging
...
// FIXED: Debug event handler dengan proper object handling
player.events.on("debug", (message) => {
    try {
        let msgStr;
        
        // FIXED: Proper object to string conversion
...
// FIXED: Button Interaction Handler dengan proper flags
        ...
        switch (interaction.customId) {
            case 'music_play_pause':
                if (!queue || !queue.isPlaying()) {
                    await interaction.reply({ content: '❌ Tidak ada lagu yang sedang diputar.', flags: 64 }); // FIXED: Use flags instead of ephemeral
                    return;
                }
...
client.on("messageCreate", async (message) => {
  ...
  // FIXED: Enhanced Play Command dengan Audio Debugging
  if (message.content.startsWith("!play ")) {
    ...
    try {
        ...
        // FIXED: Enhanced play options dengan audio debugging
        const { track } = await player.play(voiceChannel, searchResult, {
            nodeOptions: {
                ...
                // FIXED: Additional audio options
...
// Dashboard API Routes (same as before)
...
<head>
    ...
    <title>🎵 Kugy Bot Dashboard - Audio Debug</title>
    <style>
...
<body>
    <div class="container">
        <h1>🎵 Kugy Bot Dashboard - Audio Debug</h1>
        ...
        <div class="status">
            ...
            <div class="debug-info">
                ...
                • Enhanced audio monitoring enabled<br>
                • Connection state tracking active<br>
                • Error recovery system online<br>
                • Debug message filtering improved
            </div>
        </div>
