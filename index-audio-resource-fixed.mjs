...
// CRITICAL FIX: Audio Resource Creation
console.log("🎵 Discord Music Bot - AUDIO RESOURCE FIXED VERSION");
...
// CRITICAL FIX: Enhanced Player Configuration untuk Audio Resource Creation
const player = new Player(client, {
    // CRITICAL: Enhanced FFmpeg configuration untuk audio resource
    ffmpeg: {
    ...
    // CRITICAL FIX: Connection options untuk audio resource
    connectionOptions: {
        selfDeaf: true,
        selfMute: false,
    },
    // CRITICAL: Force audio resource creation
    skipFFmpeg: false,
    useLegacyFFmpeg: false,
    bufferingTimeout: 10000, // Increased to 10 seconds
    disableVolume: false,
    // CRITICAL: Additional options untuk audio resource
    leaveOnEnd: false,
    leaveOnStop: false,
    leaveOnEmpty: false,
...
// CRITICAL FIX: Enhanced Event Handlers dengan Audio Resource Force Creation
player.events.on("playerStart", (queue, track) => {
    ...
    // CRITICAL: Audio Resource Validation dengan Force Creation
    const audioResource = queue.node.resource;
    console.log(`🎵 Audio Resource: ${audioResource ? 'Created ✅' : 'Missing ❌'}`);
    
    if (!audioResource) {
        console.error(`❌ CRITICAL: Audio Resource not created! Attempting force creation...`);
        
        // CRITICAL FIX: Force audio resource creation
        setTimeout(async () => {
            try {
                console.log(`🔄 Attempting to force create audio resource...`);
                
                // Method 1: Try to recreate the queue
                const newQueue = player.queues.get(queue.metadata.guild.id);
                if (newQueue && newQueue.currentTrack) {
                    console.log(`🔄 Method 1: Recreating queue...`);
                    
                    // Save current track
                    const currentTrack = newQueue.currentTrack;
                    
                    // Skip current and re-add
                    newQueue.skip();
                    
                    setTimeout(() => {
                        newQueue.insertTrack(currentTrack, 0);
                        console.log(`✅ Track re-inserted for audio resource creation`);
                    }, 2000);
                }
            } catch (error) {
                console.error(`❌ Failed to force create audio resource:`, error);
                
                // Method 2: Try to restart the entire playback
                try {
                    console.log(`🔄 Method 2: Restarting playback...`);
                        ...
                        if (channel) {
                            ...
                            setTimeout(async () => {
                                try {
                                    ...
                                    console.error(`❌ Failed to restart playback:`, restartError);
                                }
                            }, 3000);
                        }
                    }
    ...
    if (queue.metadata && queue.metadata.channel) {
        ...
        // Send warning if audio resource missing
        if (!audioResource) {
            queue.metadata.channel.send(`⚠️ **Audio Resource Missing!** Mencoba force creation...\n💡 Jika tidak ada suara, bot akan auto-retry dalam beberapa detik.`);
        }
    }
});

// CRITICAL: Enhanced Audio Resource Monitoring
player.events.on("audioTrackAdd", (queue, track) => {
    ...
    // CRITICAL: Monitor audio resource creation after track add
    setTimeout(() => {
        const audioResource = queue.node.resource;
        console.log(`🎵 Audio resource after track add: ${audioResource ? 'Created' : 'Still Missing'}`);
        
        if (!audioResource && queue.isPlaying()) {
            console.error(`❌ CRITICAL: Audio resource still missing after track add!`);
        }
    }, 2000);
});
...
player.events.on("playerFinish", (queue, track) => {
    ...
    // CRITICAL: Check why audio finished so quickly
    const connection = queue.connection;
    const audioResource = queue.node.resource;
    ...
    // CRITICAL: Check if track finished too quickly (less than 10 seconds)
    const now = Date.now();
    if (!track.startTime) track.startTime = now;
    const playDuration = now - track.startTime;
    
    if (playDuration < 10000) { // Less than 10 seconds
        console.error(`❌ CRITICAL: Track finished too quickly! Duration: ${playDuration}ms`);
        console.log(`🔍 Possible causes:`);
        console.log(`   - Audio resource creation failed`);
        console.log(`   - FFmpeg processing error`);
        console.log(`   - Voice connection instability`);
        console.log(`   - YouTube stream issues`);
        
        if (queue.metadata && queue.metadata.channel) {
            queue.metadata.channel.send(`⚠️ **Lagu selesai terlalu cepat!**\n⏱️ Durasi: ${Math.round(playDuration/1000)} detik\n🔄 Mencoba play ulang...`);
        }
        
        // Auto-retry if finished too quickly
        setTimeout(() => {
            try {
                queue.insertTrack(track, 0);
                console.log(`🔄 Auto-retry: Track re-added due to quick finish`);
            } catch (error) {
                console.error(`❌ Failed to auto-retry:`, error);
            }
        }, 2000);
        
        return; // Don't continue with normal finish logic
    }
...
// CRITICAL FIX: Enhanced error handling untuk Audio Resource Issues
player.events.on("error", (queue, error) => {
    ...
    // CRITICAL: Handle audio resource creation errors
    if (error.message.includes('audio') || error.message.includes('resource') || error.message.includes('ffmpeg')) {
        console.error(`❌ CRITICAL: Audio Resource Creation Error!`);
        console.log(`🔧 Possible causes:`);
        console.log(`   - FFmpeg processing failure`);
        console.log(`   - Audio stream corruption`);
        console.log(`   - YouTube stream unavailable`);
        console.log(`   - Voice connection instability`);
        
        if (queue.metadata && queue.metadata.channel) {
            queue.metadata.channel.send(`❌ **Audio Resource Error!**\n🔧 Kemungkinan penyebab:\n• FFmpeg processing failure\n• Audio stream corruption\n• YouTube stream unavailable\n\n💡 **Solusi:**\n• Coba \`!play\` lagi dengan lagu berbeda\n• Restart bot jika masalah berlanjut`);
        }
        
        // Auto-retry dengan track berbeda jika ada
        if (queue && queue.tracks.size > 0) {
            console.log(`🔄 Auto-retry: Playing next track due to audio error...`);
            setTimeout(() => {
                try {
                    queue.skip();
                } catch (skipError) {
                    console.error(`❌ Error skipping after audio error:`, skipError);
                }
            }, 3000);
        }
    }
...
player.events.on("playerError", (queue, error) => {
    ...
    console.log(`🔍 Error details:`, {
        ...
        audioResource: queue?.node?.resource ? 'Active' : 'Inactive',
...
// CRITICAL FIX: Enhanced Connection state monitoring dengan Audio Resource tracking
player.events.on("connectionCreate", (queue) => {
    ...
    // CRITICAL: Enhanced audio resource validation after connection
    setTimeout(() => {
        const audioResource = queue.node.resource;
    ...
    }, 7000); // Increased timeout for audio resource validation
});
...
player.events.on("debug", (message) => {
    try {
        ...
        if (!msgStr.includes('[YOUTUBEJS]') && 
            ...
            !msgStr.includes('Unable to find matching run') &&
...
client.once("ready", () => {
  ...
  console.log(`🎵 Audio resource force creation enabled`);
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content === "!audio") {
    ...
    const embed = new EmbedBuilder()
        .setColor(queue?.node?.resource ? '#00FF00' : '#FF0000')
        .setTitle('🎵 Audio Resource Status');
    
    if (queue) {
        const connection = queue.connection;
        const audioResource = queue.node.resource;
        embed.addFields(
            { name: '🔗 Voice Connection', value: connection?.state?.status || 'Not Connected', inline: true },
            { name: '🎵 Audio Resource', value: audioResource ? '✅ Active' : '❌ Missing', inline: true },
            { name: '🎶 Current Track', value: queue.currentTrack?.title || 'None', inline: true },
            { name: '📊 Queue Size', value: queue.tracks.size.toString(), inline: true },
            { name: '▶️ Is Playing', value: queue.isPlaying() ? '✅ Yes' : '❌ No', inline: true },
            { name: '⏸️ Is Paused', value: queue.node.isPaused() ? '✅ Yes' : '❌ No', inline: true }
        );
        
        if (audioResource) {
            embed.addFields({ 
                name: '🔊 Audio Details', 
                value: `Readable: ${audioResource.readable}\nEnded: ${audioResource.ended}\nVolume: ${audioResource.volume?.volume || 'N/A'}`, 
                inline: false 
            });
        }
    } else {
  ...
  if (message.content === "!help") {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '🔧 Debug Commands', value: '• Enhanced audio resource monitoring\n• FFmpeg validation\n• Audio resource force creation\n• Quick finish detection', inline: false }
        );
  ...
  // CRITICAL FIX: Enhanced Play Command dengan Audio Resource Force Creation
  if (message.content.startsWith("!play ")) {
    ...
    try {
        ...
        searchResult.tracks[0].startTime = Date.now();

        // CRITICAL FIX: Enhanced play options dengan Audio Resource Force Creation
        const { track } = await player.play(voiceChannel, searchResult, {
            nodeOptions: {
                ...
                leaveOnEmpty: false,
                leaveOnEmptyCooldown: 30000,
                // CRITICAL: Enhanced audio resource options
                bufferingTimeout: 10000, // Increased to 10 seconds
                disableVolume: false,
                skipFFmpeg: false,
                // Force audio resource creation
        ...
        // CRITICAL: Enhanced validation dengan audio resource force creation
        setTimeout(() => {
            const connection = queue.connection;
            const audioResource = queue.node.resource;
...
// Dashboard API Routes (same as before but with audio resource status)
app.get('/api/status', (req, res) => {
    ...
    res.json({
        ...
        audioResourceStatus: queue?.node?.resource ? 'active' : 'missing'
    });
});
...
<head>
    ...
    <title>🎵 Kugy Bot Dashboard - Audio Resource Fixed</title>
    <style>
...
<body>
    <div class="container">
        <h1>🎵 Kugy Bot Dashboard - Audio Fixed</h1>
        ...
        <div class="status">
            ...
            <div class="audio-status">
                ...
                <strong>🎵 Audio Resource:</strong> <span id="audio-resource">Checking...</span><br>
                ...
                • Enhanced audio resource force creation<br>
                • Quick finish detection and auto-retry<br>
                • Audio resource validation monitoring<br>
    ...
    <script>
        ...
        async function updateStatus() {
            try {
                ...
                const audioResource = document.getElementById('audio-resource');
                const resourceStatus = data.audioResourceStatus || 'missing';
                
                if (resourceStatus === 'active') {
                    audioResource.innerHTML = '<span class="status-ok">✅ Active</span>';
                } else {
                    audioResource.innerHTML = '<span class="status-error">❌ Missing</span>';
                }
                ...
                if (data.currentTrack || data.queue.length > 0) {
                    ...
                    if (data.currentTrack) {
                        html += \`<div style="padding: 10px; background: rgba(255,152,0,0.2); border-radius: 8px; margin: 5px 0;">
                            ...
                            <br><small>🔗 Connection: \${connectionStatus} | 🎵 Audio: \${resourceStatus}</small>
                        </div>\`;
                    }
