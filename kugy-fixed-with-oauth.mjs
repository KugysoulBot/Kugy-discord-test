...
// Create cookies.txt file if it doesn't exist
...
#HttpOnly_.youtube.com	TRUE	/	TRUE	1779118488	__Secure-1PSIDTS	sidts-CjEBjplskM6jqfBhfpPoQnOBUfE4KwObmOKqkS4xtUpvyxFU0dwTznKVuLSyxA3D_nLqEAA
#HttpOnly_.youtube.com	TRUE	/	TRUE	1779118488	__Secure-3PSIDTS	sidts-CjEBjplskM6jqfBhfpPoQnOBUfE4KwObmOKqkS4xtUpvyxFU0dwTznKVuLSyxA3D_nLqEAA
#HttpOnly_.youtube.com	TRUE	/	FALSE	1782142488	HSID	Ay3F-3kw45tyw2P1W
#HttpOnly_.youtube.com	TRUE	/	TRUE	1782142488	SSID	A17nT7x8Q3JJWhRCx
.youtube.com	TRUE	/	FALSE	1782142488	APISID	KBWkqlod3a5GQD8J/AcU1KWjF_aaiZxpCk
.youtube.com	TRUE	/	TRUE	1782142488	SAPISID	w7cdTUKqIj2l6Wbo/AU1k4mJwlx_P2hFpc
.youtube.com	TRUE	/	TRUE	1782142488	__Secure-1PAPISID	w7cdTUKqIj2l6Wbo/AU1k4mJwlx_P2hFpc
.youtube.com	TRUE	/	TRUE	1782142488	__Secure-3PAPISID	w7cdTUKqIj2l6Wbo/AU1k4mJwlx_P2hFpc`;
  
  fs.writeFileSync(cookiesPath, cookiesContent);
  console.log("Created cookies.txt file for YouTube access");
}
...
// Function to get fresh YouTube cookies using yt-dlp
async function refreshYouTubeCookies() {
  try {
    ...
    const tempProfileDir = join(__dirname, "temp_browser_profile");
    ...
    // Use yt-dlp to extract cookies from a browser session
    console.log("Please sign in to YouTube in the browser window that will open...");
    console.log("After signing in, close the browser and return to this terminal.");
    
    // Open a browser for the user to log in
    await open("https://www.youtube.com/signin");
    ...
    await new Promise(resolve => {
      rl.question("Press Enter after you've signed in and closed the browser...", () => {
        rl.close();
        resolve();
      });
    });
    ...
    execSync(`yt-dlp --cookies-from-browser chrome --cookies ${cookiesPath} https://www.youtube.com`, { stdio: "inherit" });
    
    console.log("YouTube cookies refreshed successfully!");
    return true;
  } catch (error) {
    console.error("Error refreshing YouTube cookies:", error);
    return false;
  }
}

// Create a new DisTube instance with proper options
// Note: leaveOnStop option has been removed as it's not needed in DisTubeOptions
...
distube
  ...
  .on("error", async (channel, error) => {
    ...
    if (error.message && error.message.includes("Sign in to confirm you're not a bot")) {
      if (channel) {
        channel.send("❌ YouTube is asking for verification. Attempting to refresh cookies...");
      }
      
      // Try to refresh YouTube cookies
      const refreshed = await refreshYouTubeCookies();
      
      if (refreshed && channel) {
        channel.send("✅ YouTube cookies refreshed. Please try playing the song again.");
      } else if (channel) {
        channel.send("❌ Failed to refresh YouTube cookies. Please try a different song or try again later.");
      }
    } else if (channel) {
  ...
  switch (command) {
    ...
    case "p":
      ...
      try {
        ...
        // Check if the error is related to YouTube bot detection
        if (error.message && error.message.includes("Sign in to confirm you're not a bot")) {
          message.reply("❌ YouTube is asking for verification. Attempting to refresh cookies...");
          
          // Try to refresh YouTube cookies
          const refreshed = await refreshYouTubeCookies();
          
          if (refreshed) {
            message.reply("✅ YouTube cookies refreshed. Please try playing the song again.");
          } else {
            message.reply("❌ Failed to refresh YouTube cookies. Please try a different song or try again later.");
          }
        } else {
    ...
    case "refresh":
      message.channel.send("🔄 Attempting to refresh YouTube cookies...");
      const refreshed = await refreshYouTubeCookies();
      if (refreshed) {
        message.channel.send("✅ YouTube cookies refreshed successfully!");
      } else {
        message.channel.send("❌ Failed to refresh YouTube cookies. Please try again later.");
      }
      break;
    ...
    case "help":
      const helpEmbed = {
        ...
        fields: [
          ...
          {
            name: "!refresh",
            value: "Refresh YouTube cookies if you're having issues with YouTube",
          },
        ],
        footer: {
...
<body>
    ...
    <div class="container">
        ...
        <button onclick="refreshYouTubeCookies()">🔄 Refresh YouTube Cookies</button>
    </div>
    ...
    <script>
        ...
        async function refreshYouTubeCookies() {
            try {
                const response = await fetch('/refresh-cookies', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    }
                ...
                const result = await response.json();
                if (result.success) {
                    alert("YouTube cookies refreshed successfully!");
                } else {
                    alert("Error: " + result.message);
                }
                ...
                alert('Failed to refresh YouTube cookies');
            }
        }
...
// Refresh YouTube cookies endpoint
app.post("/refresh-cookies", async (req, res) => {
  try {
    ...
    if (refreshed) {
      ...
      res.json({ success: false, message: "Failed to refresh YouTube cookies" });
    }
  } catch (error) {
    console.error("Refresh cookies error:", error);
    res.json({ success: false, message: error.message });
